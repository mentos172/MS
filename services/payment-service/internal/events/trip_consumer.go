...
//rabbitmq — обертка для работы с RabbitMQ (методы для публикации и потребления).
//service — бизнес-логика, в частности для создания платежных сессий.
...
// Конструктор, возвращающий готовый к использованию экземпляр.
...
//Потребление сообщений из очереди PaymentTripResponseQueue.
//Внутри — передается анонимная функция, которая вызывается для каждого сообщения.
